...
def run_cross_validation(num_folds=5, epochs=300, checkpoint_interval=10, resume_from_last_checkpoint=False):
    ...
    # Define the augmentation configuration file path
    AUGMENTATION_CFG = 'Model/custom_hyp_s.yaml'

    # --- GLOBAL AUGMENTATION CHECK AND CONFIRMATION MESSAGE ---
    if os.path.exists(os.path.join(current_dir, AUGMENTATION_CFG)):
        print(f"\n AUGMENTATION STATUS: Custom configuration '{AUGMENTATION_CFG}' will be used for all folds.")
    else:
        print(f"\n WARNING: Augmentation file '{AUGMENTATION_CFG}' not found. Training will use default hyperparameters.")
    ...
    for fold in range(1, num_folds + 1): 
        ...
        # Set the configuration path if the file exists, otherwise use None (default)
        cfg_path = AUGMENTATION_CFG if os.path.exists(os.path.join(current_dir, AUGMENTATION_CFG)) else None

        if resume_flag:
            # Resuming: omit project/name to continue in the existing directory
...
if __name__ == '__main__':
    ...
    run_cross_validation(num_folds=5, epochs=300, checkpoint_interval=10, resume_from_last_checkpoint=False)